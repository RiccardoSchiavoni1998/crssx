...
def evaluator(p, n, k, z, l, set_, mode, current_best, w0, alpha_d, epsilon, delta, zD):
    ...
    if t>0 and l>t:
        ...
        list_base = comb(len_vb, u2+d2)*comb(u2+d2, u2)*(z**u2)*(zD**d2)
        c_base = (4*list_base*(t*log(p,2)+u2*log(z,2)+d2*log(zD,2)))
        
        memory_base = list_base*(u2*log(z,2)+d2*log(zD,2)) 
        
        list_1 = (list_base*list_base)/(p**t)      
        c_lv1 = (2*list_1)*log(p,2)
        
        num_r0 = p**(l-t)

        if mode =="nn":
            list_1 = list_1/num_r0
       
        memory_1 = list_1*(u1*log(z, 2)+delta*log(zD, 2))

        memory_cost = max(memory_base, memory_1)
        
        if mode == "bf":
            c_0 = ((list_1*list_1)/num_r0)*log(p,2)
            ...
            if partial_cost <= current_best:
                ...
                nn_costs, c_0 = find_best_nn(k, l, epsilon+delta, u1+delta, u1+delta, list_1, list_1, current_best)
        ...
        if flag:
            ...
            res['Details'] = save_details(rep, num_r0, prob, list_base, list_1, list_1, c_base, c_lv1, c_0)
...
def bjmm_2LV_fast(p, n, k, z, params):
    data = {}
    current_best = n+40
