...
def cost_evaluator(p, n, k, z, zD, l, u0, d0, ε1, δ1, ε2, δ2, alpha_e, alpha_d, set_type, current_best, mode):
    ...
    len_v_base1 = floor((k+l)/2)
    len_v_base2 = ceil((k+l)/2)
    ...
    for i in range(1, 3):
        ...
        if len_list_lv2_a<=0 or len_list_lv2_b<=0:
            return res, current_best
    ...
    if mode == "NN":
        len_list_lv1_a = len_list_lv1_a/p**(l-l0-l1)
        len_list_lv1_b = len_list_lv1_b/p**(l-l0-l1)
    ...
    if mode == "BF":
        ...
        if partial_cost < current_best:
            ...
            #NEAREST NEIGHBOUR
            nn_costs, time_cost_0 = find_best_nn(k, l, ε1 + δ1 + ε2 + δ2, u_a+d_a, u_b+d_b, len_list_lv1_a, len_list_lv1_b, current_best)
    ...
    if flag:    
        len_list_base = max(len_list_base_values.values())
        len_list_lv2 = max(len_list_lv2_values.values())
...
#p, n, k, z, alpha_d, zD, alpha_e, min_epsilon, max_epsilon, min_delta, max_delta, set_, mode, current_best, sec_level = 0, verb = False
#SEBASTIAN RANGES
def bjmm_3lv_E(p, n, k, z, zD, alpha_e, alpha_d, mode, current_best, sec_level = 999, verb=False):
    
    #current_best = n+40
    if verb: print("Start", "Bjmm 3lv on Shifted set", mode, "on Params:", p, n, k, z)
    list_cost = []
            ...
            for ε2 in range(0, max_ε2+1):
                ...
                for δ1 in range(0, min(max_δ1, 17)+1):
                    ...
                    for δ2 in range(0, min(max_δ2, 9)+1):
                        ...
                        res_details, cost = cost_evaluator(p, n, k, z, zD, l, u0, d0, ε1, δ1, ε2, δ2, alpha_e, alpha_d, "E", current_best, mode)
    ...
    if verb: print("End", "Bjmm 3lv on Shifted set", mode, "Params:", p, n, k, z, "Secure")
...
def bjmm_3lv_shifted(p, n, k, z, zD, alpha_e, alpha_d, mode, current_best, sec_level = 999, verb=False):
        ...
        for u0 in range(8, k+l+1):
            ...
            for δ1 in range(0, max_δ1+1):
                ...
                for δ2 in range(0, max_δ2+1):
                    ...
                    if len(res_details) != {} and cost < current_best:
                        if cost < sec_level:
                            if verb: print("Finito", "Bjmm 3lv on Shifted set", mode, "on Params:", p, n, k, z, "Not Secure")
                            return cost, [], False
                        current_best = cost
                        list_cost.append(res_details)
                        res_details = {}
    ...
    if verb: print("Finito", "Bjmm 3lv on Shifted set", mode, "on Params:", p, n, k, z, "Secure")
    return current_best, list_cost, True                
